...
[_LOWER] = LAYOUT_ortho_4x12( \
  ...
  _______, KC_F5,   KC_F6,   KC_F7,   KC_F8,   XXXXXXX, KC_PAST, KC_P4,   KC_P5, KC_P6,   KC_PPLS, KC_DEL,  \
  _______, KC_F9,   KC_F10,  KC_F11,  KC_F12,  XXXXXXX, KC_PSLS, KC_P1,   KC_P2, KC_P3,   KC_PMNS, _______, \
  SCN_LCK, _______, _______, _______, _______, _______, _______, _______, KC_P0, KC_PDOT, SPOT,    XXXXXXX
),
...
[_RAISE] = LAYOUT_ortho_4x12( \
  ...
  _______, LOGW,    TEAMS,   CHROME,  TERM,    OUTLOOK, KC_EQL,  KC_MINS, KC_BSLS, KC_LBRC, KC_RBRC, KC_GRV,  \
  _______, SH_CLRH, SH_CLRE, SH_HOME, SH_END,  SH_RSCH, KC_MPLY, KC_MUTE, KC_VOLD, KC_VOLU, SCN_SHT, _______, \
  SCN_LCK, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______, _______
),
  ...
  if (record->event.pressed) {
    switch(keycode) {
      ...
      case CHROME:  // Open Chrome
        ...
        return false;
      case LOGW:  // Open Jira log work window
        ...
        SEND_STRING(SS_TAP(X_ENTER));
        return false;
      case MIN: // Hide and Minimize all windows
        SEND_STRING(SS_LGUI(SS_LALT(SS_TAP(X_H)SS_TAP(X_M))));
        return false;
      case OUTLOOK:  // Open MS Outlook
        SEND_STRING(SS_LGUI(SS_TAP(X_SPACE)));
        _delay_ms(100);
        SEND_STRING("Microsoft Outlook");
        _delay_ms(100);
        SEND_STRING(SS_TAP(X_ENTER));
        return false;
      case SPOT:  // Open Spotify
        ...
        SEND_STRING(SS_TAP(X_ENTER));
        return false;
      case TEAMS: // Open Microsoft Teams
        SEND_STRING(SS_LGUI(SS_TAP(X_SPACE)));
        _delay_ms(100);
        SEND_STRING("Microsoft Teams");
        _delay_ms(100);
        SEND_STRING(SS_TAP(X_ENTER));
        return false;
      case TERM:  // Open Terminal
        SEND_STRING(SS_LGUI(SS_TAP(X_SPACE)));
        _delay_ms(100);
